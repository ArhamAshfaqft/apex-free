...
final class Loader {
	...
	public function init() {
		...
		// Register Container Carousel extension hooks (only if active)
		if ( self::is_addon_active( 'container_carousel' ) ) {
			add_action( 'elementor/element/container/section_layout/after_section_end', [ $this, 'register_container_carousel_controls' ], 10, 2 );
			add_action( 'elementor/frontend/container/before_render', [ $this, 'before_element_render' ] );
			add_action( 'elementor/preview/enqueue_scripts', [ $this, 'enqueue_carousel_editor_assets' ] );
		}
	...
	public function get_addons() {
		$addons = [
			...
			// Free Extensions
			'container_carousel' => [
				'title' => esc_html__( 'Container Carousel', 'apex-addons-for-elementor' ),
				'category' => 'extensions',
				'desc' => esc_html__( 'Convert any Elementor Container into a smooth continuous carousel.', 'apex-addons-for-elementor' ),
	...
	/**
	 * Register Container Carousel controls section on Container elements
	 */
	public function register_container_carousel_controls( $element, $section_id ) {
		$element->start_controls_section(
			'section_eas_container_carousel',
			[
				'label' => esc_html__( 'Apex Container Carousel', 'apex-addons-for-elementor' ),
				'tab'   => \Elementor\Controls_Manager::TAB_LAYOUT,
			]
		);

		$element->add_control(
			'eas_container_carousel',
			[
				'label'        => esc_html__( 'Convert Container to Carousel', 'apex-addons-for-elementor' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'label_on'     => esc_html__( 'Yes', 'apex-addons-for-elementor' ),
				'label_off'    => esc_html__( 'No', 'apex-addons-for-elementor' ),
				'return_value' => 'yes',
		...
		$element->add_responsive_control(
			'eas_carousel_slide_width',
			[
				'label'      => esc_html__( 'Carousel Slide Width', 'apex-addons-for-elementor' ),
				'type'       => \Elementor\Controls_Manager::SLIDER,
				'size_units' => [ 'px', '%', 'vw' ],
				'range'      => [
				...
				'selectors'  => [
					'{{WRAPPER}}.eas-container-carousel-active .eas-carousel-slide' => 'width: {{SIZE}}{{UNIT}} !important; flex-shrink: 0 !important;',
					'{{WRAPPER}}.eas-carousel-editor-preview > .e-con-inner > .elementor-element' => 'width: {{SIZE}}{{UNIT}} !important; flex-shrink: 0 !important;',
					'{{WRAPPER}}.eas-carousel-editor-preview > .elementor-container > .elementor-element' => 'width: {{SIZE}}{{UNIT}} !important; flex-shrink: 0 !important;',
					'{{WRAPPER}}.eas-carousel-editor-preview > .elementor-element' => 'width: {{SIZE}}{{UNIT}} !important; flex-shrink: 0 !important;',
				],
				'condition'  => [
					'eas_container_carousel' => 'yes',
				],
			]
		);
		...
		$element->add_control(
			...
			[
				...
				'size_units' => [ 'px' ],
				'range'      => [
					'px' => [ 'min' => 1000, 'max' => 15000, 'step' => 100 ],
				],
				'default'    => [
					'size' => 5000,
				],
				'description' => esc_html__( 'Transition duration in milliseconds. Higher values scroll slower/smoother.', 'apex-addons-for-elementor' ),
				'condition'  => [
					'eas_container_carousel' => 'yes',
				],
			]
		...
		$element->add_control(
			...
			[
				...
				'options'   => [
					...
					'btt' => esc_html__( 'Bottom to Top', 'apex-addons-for-elementor' ),
					'ttb' => esc_html__( 'Top to Bottom', 'apex-addons-for-elementor' ),
				],
				'condition' => [
					'eas_container_carousel' => 'yes',
				],
		...
		$element->add_responsive_control(
			'eas_carousel_height',
			[
				'label'      => esc_html__( 'Carousel Height', 'apex-addons-for-elementor' ),
				'type'       => \Elementor\Controls_Manager::SLIDER,
				'size_units' => [ 'px', 'vh', 'em' ],
				'range'      => [
					'px' => [ 'min' => 100, 'max' => 1000, 'step' => 1 ],
				...
				'selectors'  => [
					'{{WRAPPER}}.eas-container-carousel-active .eas-carousel-swiper.swiper-vertical' => 'height: {{SIZE}}{{UNIT}} !important;',
					'{{WRAPPER}}.eas-carousel-editor-preview.eas-carousel-vertical > .e-con-inner' => 'height: {{SIZE}}{{UNIT}} !important;',
					'{{WRAPPER}}.eas-carousel-editor-preview.eas-carousel-vertical > .elementor-container' => 'height: {{SIZE}}{{UNIT}} !important;',
					'{{WRAPPER}}.eas-carousel-editor-preview.eas-carousel-vertical' => 'height: {{SIZE}}{{UNIT}} !important;',
				],
				'condition'  => [
					'eas_container_carousel' => 'yes',
					'eas_carousel_direction' => [ 'btt', 'ttb' ],
				],
			]
		);

		$element->add_responsive_control(
			'eas_carousel_slides_to_show',
			[
				'label'      => esc_html__( 'Slides to Show', 'apex-addons-for-elementor' ),
				'type'       => \Elementor\Controls_Manager::SLIDER,
				'range'      => [
					'px' => [ 'min' => 1, 'max' => 10, 'step' => 0.1 ],
				],
				'default'    => [
					'size' => 4,
				],
				'condition'  => [
					'eas_container_carousel' => 'yes',
					'eas_carousel_direction' => [ 'rtl', 'ltr' ],
				],
			]
		);

		$element->add_control(
			'eas_carousel_pause_on_hover',
			[
		...
		$this->add_companion_features_notice(
			$element,
			'apexadfo_container_carousel_companion_notice',
			esc_html__( 'Scroll-linked carousel parallax and advanced depth movement.', 'apex-addons-for-elementor' ),
			[ 'eas_container_carousel' => 'yes' ]
		);
	...
	/**
	 * Inject config attributes and assets in frontend renderer
	 */
	public function before_element_render( $element ) {
		$settings = $element->get_settings_for_display();

		if ( ! empty( $settings['eas_container_carousel'] ) && 'yes' === $settings['eas_container_carousel'] ) {
			// Enqueue assets dynamically
			wp_enqueue_script( 'apexadfo-container-carousel-js' );
			wp_enqueue_style( 'apexadfo-container-carousel-css' );

			$desktop_slides = floatval( $settings['eas_carousel_slides_to_show']['size'] ?? 4 );
			$tablet_slides  = floatval( $settings['eas_carousel_slides_to_show_tablet']['size'] ?? $desktop_slides );
			$mobile_slides  = floatval( $settings['eas_carousel_slides_to_show_mobile']['size'] ?? $tablet_slides );

			$carousel_config = [
				'speed'         => intval( $settings['eas_carousel_speed']['size'] ?? 5000 ),
				'direction'     => esc_attr( $settings['eas_carousel_direction'] ?? 'rtl' ),
				'pauseOnHover'  => esc_attr( $settings['eas_carousel_pause_on_hover'] ?? 'yes' ),
				'slidesDesktop' => $desktop_slides,
				'slidesTablet'  => $tablet_slides,
				'slidesMobile'  => $mobile_slides,
			];

			$element->add_render_attribute( '_wrapper', 'data-eas-carousel-config', wp_json_encode( $carousel_config ) );
			$element->add_render_attribute( '_wrapper', 'data-eas-pause-on-hover', esc_attr( $settings['eas_carousel_pause_on_hover'] ?? 'yes' ) );
			$element->add_render_attribute( '_wrapper', 'data-eas-carousel-disable-tablet', esc_attr( $settings['eas_carousel_disable_tablet'] ?? 'no' ) );
			$element->add_render_attribute( '_wrapper', 'data-eas-carousel-disable-mobile', esc_attr( $settings['eas_carousel_disable_mobile'] ?? 'no' ) );
			$element->add_render_attribute( '_wrapper', 'class', 'eas-container-carousel-active' );
		}
	}
	...
	/**
	 * Enqueue carousel assets inside Elementor editor preview iframe
	 */
	public function enqueue_carousel_editor_assets() {
		wp_enqueue_style(
			'apexadfo-container-carousel-css',
			plugins_url( 'assets/css/container-carousel.css', __FILE__ ),
			[],
		...
		wp_enqueue_script(
			'apexadfo-container-carousel-editor-js',
			plugins_url( 'assets/js/container-carousel-editor.js', __FILE__ ),
			[ 'jquery', 'apexadfo-container-carousel-js' ],
			'1.0.22',
			true
		);
	}
